...
class ManagerController extends BaseController
{
    ...
    {   
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            try {
                $fs->mkdir($directory);
                $this->addFlash('success', $translator->trans('Creación de carpeta exitosa'));
            } catch (IOExceptionInterface $e) {
                $this->addFlash('danger', $translator->trans('No se pudo crear la carpeta', ['%message%' => $data['name']]));
            }
        ...
        if ($formRename->isSubmitted() && $formRename->isValid()) {
            ...
            if (isset($data['name']) && $NewfileName !== $fileName) {
                ...
                if (strpos($NewfilePath, $fileManager->getCurrentPath()) !== 0) {
                    $this->addFlash('danger', $translator->trans('Renombramiento de archivo no autorizado'));
                } else {
                    $fs = new Filesystem();
                    try {
                        $fs->rename($OldfilePath, $NewfilePath);
                        $this->addFlash('success', $translator->trans('Archivo renombrado'));
                        //File has been renamed successfully
                    } catch (IOException $exception) {
                        $this->addFlash('danger', $translator->trans('Error, no se pudo renombrar'));
                    }
                }
            } else {
                $this->addFlash('warning', $translator->trans('Archivo no renombrado'));
            }
        }
            ...
            if (isset($queryParameters['delete'])) {
                ...
                foreach ($queryParameters['delete'] as $fileName) {
                    ...
                    if (strpos($filePath, $fileManager->getCurrentPath()) !== 0) {
                        $this->addFlash('danger', $translator->trans('Eliminar archivos'));
                    } else {
                        ...
                        try {
                            ...
                            $this->addFlash('danger', $translator->trans('Eliminación de archivo no autorizado'));
                        }
                        $this->dispatch(FileManagerEvents::POST_DELETE_FILE);
                    }
                }
                if ($is_delete) {
                    $this->addFlash('success', $translator->trans('Archivo eliminado'));
                }
                ...
                try {
                    ...
                    $this->addFlash('success', $translator->trans('Carpeta eliminada'));
                } catch (IOException $exception) {
                    $this->addFlash('danger', $translator->trans('Eliminación de carpeta no autorizada'));
                }
